...
describe("Router", () => {
  ...
  describe("add", () => {
    ...
    it("should support multiple routes", async () => {
      ...
      const handler1 = jest
        .fn()
        .mockResolvedValue(new Response("Response 1", { status: 200 }));

      const handler2 = jest
        .fn()
        .mockResolvedValue(new Response("Response 2", { status: 201 }));
      ...
      // Test route 1
      const request1 = new Request("https://example.com/route1", {
        method: "GET",
      });
      ...
      // Test route 2
      const request2 = new Request("https://example.com/route2", {
        method: "POST",
      });
  ...
  describe("handle", () => {
    it("should handle wildcards in routes", async () => {
      ...
      const mockHandler = jest
        .fn()
        .mockResolvedValue(new Response("Wildcard response", { status: 200 }));

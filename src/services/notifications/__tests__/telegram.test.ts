...
describe("TelegramNotifier", () => {
  ...
  describe("sendSummary", () => {
    ...
    it("should return false if not configured", async () => {
      ...
      const result = await unconfiguredNotifier.sendSummary(
        mockStory,
        mockSummary,
      );
      ...
      // Verify the log
      expect(logger.warn).toHaveBeenCalledWith(
        "Telegram notifier not properly configured",
      );
    });
  ...
  describe("formatSummary", () => {
    ...
    it("should properly escape markdown characters", () => {
      ...
      const summaryWithMarkdown = {
        ...mockSummary,
        summary:
          "Summary with *bold* and _italic_ and [link](https://example.com)",
      };
      ...
      expect(result).toContain(
        "Summary with \\*bold\\* and \\_italic\\_ and \\[link\\]\\(https://example\\.com\\)",
      );
    });
  });
});

...
describe("ContentCleaner", () => {
  describe("clean", () => {
    ...
    it("should remove common website clutter", () => {
      const input =
        "Accept cookies to continue. Privacy policy. Subscribe to our newsletter! Log in to comment.";
      const expected =
        "Accept  to continue. . Subscribe to our ! Log in to comment.";
      expect(cleaner.clean(input)).toBe(expected);
    });
  ...
  describe("extractExcerpt", () => {
    ...
    it("should extract an excerpt with the default length", () => {
      const input =
        "This is the first sentence. This is the second sentence. This is the third sentence. This is a very long fourth sentence that should exceed the default length limit when combined with the previous sentences.";

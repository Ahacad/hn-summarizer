...
export async function notificationSenderHandler(
  ...
  try {
    ...
    const stories = await storyRepo.getStoriesByStatus(ProcessingStatus.COMPLETED, 5);
    logger.info('Found completed stories for notification', { count: stories.length });
    ...
    for (const story of stories) {
      try {
        ...
        if (!summary) {
          logger.warn('Failed to retrieve summary', { storyId: story.id, summaryId: story.summaryId });
          results.telegram.skipped++;
          results.discord.skipped++;
          continue;
        }
        ...
        if (telegramNotifier.isConfigured()) {
          try {
            ...
            if (success) {
              ...
              logger.warn('Failed to send summary to Telegram', { storyId: story.id });
            }
          } catch (error) {
            results.telegram.failed++;
            logger.error('Error sending to Telegram', { error, storyId: story.id });
          }
        } else {
          results.telegram.skipped++;
        }
        ...
        if (discordNotifier.isConfigured()) {
          try {
            ...
            if (success) {
              ...
              logger.warn('Failed to send summary to Discord', { storyId: story.id });
            }
          } catch (error) {
            results.discord.failed++;
            logger.error('Error sending to Discord', { error, storyId: story.id });
          }
        } else {
          results.discord.skipped++;
        }
        ...
        logger.error('Error processing notification for story', { error, storyId: story.id });
        results.telegram.failed++;
        results.discord.failed++;
      }
    }

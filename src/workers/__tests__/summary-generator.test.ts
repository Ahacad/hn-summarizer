...
describe('Summary Generator Worker', () => {
  ...
  it('should generate summaries for extracted stories', async () => {
    ...
    // Call the handler
    const response = await summaryGeneratorHandler(request, {}, new ExecutionContext());
    ...
    expect(mockGetStoriesByStatus).toHaveBeenCalledWith(ProcessingStatus.EXTRACTED, 5);
    
    // Verify status updates
    expect(mockUpdateStatus).toHaveBeenCalledWith(1, ProcessingStatus.SUMMARIZING);
    expect(mockUpdateStatus).toHaveBeenCalledWith(2, ProcessingStatus.SUMMARIZING);
    ...
    expect(mockUpdateStatus).toHaveBeenCalledWith(1, ProcessingStatus.COMPLETED);
    expect(mockUpdateStatus).toHaveBeenCalledWith(2, ProcessingStatus.COMPLETED);
  ...
  it('should handle missing content ID', async () => {
    ...
    expect(mockUpdateStatus).toHaveBeenCalledWith(3, ProcessingStatus.FAILED, 'No content ID available');
  ...
  it('should handle content retrieval failures', async () => {
    ...
    expect(mockUpdateStatus).toHaveBeenCalledWith(1, ProcessingStatus.COMPLETED);
    expect(mockUpdateStatus).toHaveBeenCalledWith(2, ProcessingStatus.FAILED, 'Failed to retrieve content');
  ...
  it('should handle summarization failures', async () => {
    ...
    // Verify appropriate status updates
    expect(mockUpdateStatus).toHaveBeenCalledWith(1, ProcessingStatus.COMPLETED);
    expect(mockUpdateStatus).toHaveBeenCalledWith(2, ProcessingStatus.FAILED, 'Error: Summarization error');
  ...
  it('should handle summary saving failures', async () => {
    ...
    // Verify appropriate status updates
    expect(mockUpdateStatus).toHaveBeenCalledWith(1, ProcessingStatus.COMPLETED);
    expect(mockUpdateStatus).toHaveBeenCalledWith(2, ProcessingStatus.FAILED, 'Failed to save summary');
  ...
  it('should handle worker-level errors', async () => {
    ...
    expect(logger.error).toHaveBeenCalledWith('Error in summary generator worker', expect.any(Object));

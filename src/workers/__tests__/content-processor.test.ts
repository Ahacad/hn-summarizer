...
describe('Content Processor Worker', () => {
  ...
  it('should process stories and extract content', async () => {
    ...
    // Call the handler
    const response = await contentProcessorHandler(request, {}, new ExecutionContext());
    ...
    expect(mockGetStoriesByStatus).toHaveBeenCalledWith(ProcessingStatus.PENDING, 10);
    ...
    expect(mockUpdateStatus).toHaveBeenCalledWith(1, ProcessingStatus.EXTRACTING);
    expect(mockUpdateStatus).toHaveBeenCalledWith(2, ProcessingStatus.EXTRACTING);
    expect(mockUpdateStatus).toHaveBeenCalledWith(3, ProcessingStatus.FAILED, 'No URL provided');
    
    // Verify content was saved
    expect(mockSaveContent).toHaveBeenCalledTimes(2);
    
    // Verify story status was updated to EXTRACTED
    expect(mockUpdateStatus).toHaveBeenCalledWith(1, ProcessingStatus.EXTRACTED);
    expect(mockUpdateStatus).toHaveBeenCalledWith(2, ProcessingStatus.EXTRACTED);
  ...
  it('should handle extraction failures', async () => {
    ...
    expect(mockUpdateStatus).toHaveBeenCalledWith(1, ProcessingStatus.EXTRACTED);
    expect(mockUpdateStatus).toHaveBeenCalledWith(2, ProcessingStatus.FAILED, 'Failed to extract content');
  ...
  it('should handle content saving failures', async () => {
    ...
    // Verify appropriate status updates
    expect(mockUpdateStatus).toHaveBeenCalledWith(1, ProcessingStatus.EXTRACTED);
    expect(mockUpdateStatus).toHaveBeenCalledWith(2, ProcessingStatus.FAILED, 'Failed to save content');
  ...
  it('should handle unexpected errors', async () => {
    ...
    expect(mockUpdateStatus).toHaveBeenCalledWith(1, ProcessingStatus.FAILED, 'Error: Unexpected error');
  ...
  it('should handle worker-level errors', async () => {
    ...
    expect(logger.error).toHaveBeenCalledWith('Error in content processor worker', expect.any(Object));

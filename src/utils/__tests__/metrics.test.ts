...
describe('Metrics', () => {
  ...
  describe('time', () => {
    it('should time a function execution', async () => {
      ...
      const allMetrics = metrics.getAll();
      expect(allMetrics[`${MetricType.LATENCY}_slow_operation`]).toBeGreaterThanOrEqual(50);
    });
    ...
    it('should handle errors in timed functions', async () => {
      ...
      await expect(metrics.time('error_operation', errorFunction)).rejects.toThrow('Test error');
